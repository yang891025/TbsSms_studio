...
public class BankSmsCallBack extends HttpCallback {
	...
	protected void onPostExecute(Integer integer) {
		...
		// String placeholder =
		// wenziUtil.QtoB(m_iniFileIO.getIniString(Constants.EXTERNALPATH +
		// Constants.INIURL, Constants.SMSGATEWAYSMSCONTENTPLACEHOLDER,
		// Constants.PLACEHOLDER, "", (byte) 0));
		// Log.d(TAG, "placeholder:"+placeholder);
		if (result) {
			// String success =
			// wenziUtil.QtoB(m_iniFileIO.getIniString(Constants.EXTERNALPATH +
			// Constants.INIURL, Constants.SMSGATEWAYSMSCONTENT,
			...
			// notifyResult =
			// "《中国大百科全书数据库》自动回复：请点击此链接<a href=\"http://m.bkzx.cn/smsearch.cbs?smword=[%=smword%]&userphone=[%=userphone%]&smcode=[%=smcode%]\">查看\"[%=smword%]\"</a>的有关内容。";
		} else {
		...
		if (jsonResult.length() > 70) {// 判断短信长度是否大于70个字，如果大于70分多条短信发送
			// List<String> contents = smsManager.divideMessage(jsonResult);//
			// 截取短信息
			// for (String sms : contents) {// 循环发送短信
			// Log.d(TAG, "length > 70 :"+sms);
			// smsManager.sendTextMessage(type.trim(), null, sms, null, null);//
			// 发送短信
			// }
			ArrayList<String> msgs = smsManager.divideMessage(jsonResult);
			Log.d(TAG, "msgs size:" + msgs.size());
			smsManager.sendMultipartTextMessage(type.trim(), null, msgs, null,
					null);
		} else {
			smsManager.sendTextMessage(type.trim(), null, jsonResult, null,
					null);// 发送一条短信
		}
	}
