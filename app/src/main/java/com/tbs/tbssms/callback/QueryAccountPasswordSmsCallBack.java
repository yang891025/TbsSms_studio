...
public class QueryAccountPasswordSmsCallBack extends HttpCallback {

	private static final String TAG = "QueryAccountPasswordSmsCallBack";
	...
	protected void onPostExecute(Integer integer) {
		...
		if(result){
			String success = wenziUtil.QtoB(m_iniFileIO.getIniString(Constants.EXTERNALPATH + Constants.INIURL, Constants.SMSGATEWAYSMSCONTENT, Constants.CXSUCCESSSMSCONTENT, "", (byte) 0));
			int count = 0;
			int start = 0;
			...
			notifyResult = wenziUtil.QtoB(m_iniFileIO.getIniString(Constants.EXTERNALPATH + Constants.INIURL, Constants.SMSGATEWAYSMSCONTENT, Constants.CXUNSUCCESSSMSCONTENT, "", (byte) 0));
		}
		Log.d(TAG, "QueryAccountPasswordSmsCallBack notifyResult:"+notifyResult);
		...
		if (notifyResult.length() > 70) {// 判断短信长度是否大于70个字，如果大于70分多条短信发送
			...
			for (String sms : contents) {// 循环发送短信
				smsManager.sendTextMessage(type.trim(), null, notifyResult, null, null);// 发送短信
			}
		} else {
			smsManager.sendTextMessage(type.trim(), null, notifyResult, null, null);// 发送一条短信
		}
	}
	...
	private boolean parseJson(Context context, String json) {
		...
		try {
			...
			if(response!= null && response.equals("查询成功")){
